...
# import xlrd
import sys
from urllib.parse import unquote, quote  # 进行URL码的转换
...
def get_index_of_list(row, features):
    """
...
def compute_with_disc(disc_list, key_list, function='add', key_add='', parameters=None, split_sign='|'):
    """
    ...
    :param parameters: function为retain时，元素在parameter内存在，才参与处理
    ...
    if function == 'add':
        ...
        sum_value = 0  # 用于计算所有的数字之后
        for key in key_list:
            try:
                sum_value += int(disc_list[key])
            except:
                except_num += 1
                sum_value += 0
        return sum_value
            ...
            if key_list != '':
                ...
                for key in key_list:
                    try:
                        ...
                        if disc_list[key] in parameters:
...
def get_dict_from_document(path, features, encoding='gbk', key_for_N=False, length=0, key_length=1, ifIndex=True,
    ...
    :param key_for_N_type:如果数据中有一个key对应多个值的情况，则多个值的集合格式，dict为集合，其他为数组
    ...
    if input_type == 'path':
        ...
        if file_type == 'csv' or file_type == 'txt':
            with open(path, encoding=encoding) as file:
                for i, row in enumerate(file):
                    ...
                    if i == 0:
                        ...
                        if ifIndex:
                            if filter_feature:  # add 2022/9/8
                                filter_index = get_index_of_list(row, filter_feature)
                            index_list = get_index_of_list(row, features)
                            continue
                    ...
                    if (len(index_list) - key_length) >= 2:
                        ...
                        if key_for_N:
                            try:
                                if type(key_for_N_type) == list:
                                    for j in range(len(key_for_N_type)):
                                        # 如果key_for_N_type为‘dict’，则将多个值以集合保存
                                        if key_for_N_type[j] == 'disc':
                                            ganlist[key_string][j].add(row[index_list[-1]])
                                        # 如果key_for_N_type不是‘dict’，则将多个值以数组保存
                                        elif key_for_N_type[j] == 'list':
                                            ganlist[key_string][j].append(row[index_list[-1]])
                                        elif key_for_N_type[j] == 'add':
                                            ganlist[key_string][j] += float(row[index_list[-1]])
                                        elif key_for_N_type[j] == 'count':
                                            ganlist[key_string][j] += 1
                                else:
                                    # 如果key_for_N_type为‘dict’，则将多个值以集合保存
                                    ...
                                    # 如果key_for_N_type不是‘dict’，则将多个值以数组保存
                                        ...
                                        ganlist[key_string] += 1

                            # 当添加失败时，先创建key-value
                            except:
                                if type(key_for_N_type) == list:
                                    for k in range(len(key_for_N_type)):
                                        if k == 0:
                                            if key_for_N_type == 'disc':
                                            ...
                                            if key_for_N_type == 'disc':
                                                ...
                                                ganlist[key_string].append(float(row[index_list[-1]]))
                                            elif key_for_N_type[j] == 'count':
                                                ganlist[key_string].append(1)
                                else:
                                    if key_for_N_type == 'disc':
                                        ganlist[key_string] = {row[index_list[-1]]}
                                    elif key_for_N_type == 'list':
            ...
            if ifIndex:
                ...
                index_list = get_index_of_list(columns, features)
            # 如果输入的features直接为索引，则直接采用
            else:
                index_list = features
            data = data.values
...
def basic_duplicate_remove(lists, element=None):
    """
    数组（一维或多维）去重
    :param lists: 一维或多维数组
...
def compute_intervel_of_two_time(time_first, time_second, return_type='minite'):
    ...
    """
    # 计算每段路门架之间的间隔时间
    interval = datetime.datetime.strptime(time_second,
                                          '%Y-%m-%d %H:%M:%S') - datetime.datetime.strptime(time_first,
                                                                                            '%Y-%m-%d %H:%M:%S')
    if return_type == 'minite':
        interval = interval.total_seconds() / 60.0
    elif return_type == 'hour':
        interval = interval.total_seconds() / 3600.0
    elif return_type == 'day':
        interval = interval.total_seconds() / 3600.0 / 24
    ...
    if return_type == 'all' or return_type == 'mean':
        ...
        for value in list_value:
            ...
            try:
                ...
                mean_value[value] = 1
        max_mode_value, max_mode_index = get_topN_values_or_index(list(mean_value.values()), 1,
                                                                  list(mean_value.keys()), True)
...
def chargeIf_prop_of_two_part(lists, index_in, parameter_value):
    """
    判断所给数组被所给下标分割的两部分比例是否大于所给的系数
    :param index_in: 输入的断点的下标位置
    :param parameter_value: 进行对比的系数值
    :param lists: 输入的数组
    ...
    if index_in != 0:
        ...
        if prop < float(1 / parameter_value):  # 判断比例是否大于所给的系数, 2022/3/1修改，将parameter更改为float(1/parameter)
            if_big = 1
    else:
        if_big = 1
    return if_big
...
def get_topN_values_or_index(value_list, top_num, index_list=None, if_index=False, treat_type='max'):
    """
...
def basic_quick_sort(values, start, end, index_list=None, element=-1):
    """
    快速排序递归函数
...
def basic_compare_disc_match_other(disc_origin, disc_target, value_list=None):
    """
...
def process_apriori(data, remove_order=False, min_support=0, min_confidence=0, K='ALL', key_name='', ifSort=True,
    ...
    # 获得输入数据的支持度、置信度和提升度
    support, confidence_first, confidence_second, upValue_first, \
        total_num = basic_apriori_new(data, min_support=min_support, min_confidence=min_confidence, K=K,
                                      key_name=key_name, ifSort=ifSort, if_inout=if_inout)
...
def get_all_feature_compose(feature, K='ALL', result_compose=None, ifSort=True, if_inout=False):
    """
    获得输入数组所有的组合可能性
    :param if_inout:是否进行in、out标识，默认为FALSE
    :param ifSort:是否考虑次序因素，默认为FALSE
...
'''
    创建时间：2022/4/8
    完成时间：2022/4/8
    功能：对输入的字符串中的URL码进行转换
    修改时间：
'''


def transform_URL_to_UTF(string, encoding='gbk'):
    """
    对输入的字符串中的URL码进行转换
    :param encoding:
    :param string: 输入的字符串，包含URL码
    :return: 输出字符串，为转换后UTF码
    """
...
'''
    创建时间：2022/11/8
    完成时间：2022/11/8
    功能：获取输入值到目标值的距离（移动步数）
    关键字：字典，标准路径，距离
    修改时间：
'''


def compute_step_between_input_and_target(input_data, standard_dict, target):
    """
    获取输入值到目标值的距离（移动步数）
    :param input_data: 输入值
    :param standard_dict: 字典
    :param target: 目标对比值
    :return:
    """
    step = 0  # 用于计算移动步长
    ...
    while True:
        ...
        if input_data[0] == target:
            ...
            for i in range(len(input_data)):
                try:
                    list_ls.extend(standard_dict[input_data[i]])
                except:
                    # 如果匹配不到，返回错误
                    continue
                # 如果下一个
                if target in list_ls:
                    return step
            input_data = []
            input_data.extend(list_ls)

            # 如果搜索步长超过了15，进行推出，返回超限
            if step > 15:
                return 'out'
...
def get_next_n_from_disc(target_string, disc_data, num, result_path=None, index_list=None, split_path=None,
                         treat_type='split'):
    """
    ...
    if treat_type == 'split':
        ...
        if type(target_string) == str:
            if len(result_path) != 0:
                return_data.extend(result_path[0])
            # 终止判定，如果到了递归次数上限num或者触发限制条件(bug:限制条件固定)
            if num < 1 or len(target_string) < 16:
                return_data.append(target_string)
                split_path.append(return_data)
                return

            # 未达到终止条件且target_string为字符串的话，则进行下一步追踪
            return_data.append(target_string)
            if len(result_path) == 0:
                result_path.append(return_data)
            else:
                result_path[0] = return_data
            try:
                target_string_new = disc_data[target_string]
            except:
                # 如果匹配不到直接返回
                return
            num += -1
            # 判断获取的target_string个数
            if len(target_string_new) == 1:
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path[:1], split_path=split_path,
                                     treat_type=treat_type)
            elif len(target_string_new) == 2:
                result_path.append(return_data)
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path[:1], split_path=split_path,
                                     treat_type=treat_type)
                get_next_n_from_disc(target_string_new[1], disc_data, num, result_path[1:], split_path=split_path,
                                     treat_type=treat_type)
            else:
                result_path.append(return_data)
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path[:1], split_path=split_path,
        ...
        # 判断是否target_string是数组
        else:
            return_data.extend(result_path[0])
            result_path.append(return_data)
            # 如果是数组且长度为2，则直接拆成两个进行下一步递归
            if len(target_string) == 2:
                get_next_n_from_disc(target_string[0], disc_data, num, result_path[:1], split_path=split_path,
                                     treat_type=treat_type)
                get_next_n_from_disc(target_string[1], disc_data, num, result_path[1:], split_path=split_path,
                                     treat_type=treat_type)
            # 如果长度大于2，则拆成出第一个和其他进行下一步递归
            else:
                get_next_n_from_disc(target_string[0], disc_data, num, result_path[:1], split_path=split_path,
                                     treat_type=treat_type)
                get_next_n_from_disc(target_string[1:], disc_data, num, result_path[1:], split_path=split_path,
        ...
        if type(target_string) == str:
            # 终止判定，如果到了递归次数上限num或者触发限制条件(bug:限制条件固定)
            if num < 1 or len(target_string) < 16:
                result_path.append(target_string)
            ...
            # 判断获取的target_string个数
            if len(target_string_new) == 1:
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path, index_list,
                                     treat_type=treat_type)
            elif len(target_string_new) == 2:
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path, index_list,
                                     treat_type=treat_type)
                get_next_n_from_disc(target_string_new[1], disc_data, num, result_path, index_list,
                                     treat_type=treat_type)
            else:
                get_next_n_from_disc(target_string_new[0], disc_data, num, result_path, index_list,
                                     treat_type=treat_type)
        ...
        # 判断是否target_string是数组
            ...
            # 如果是数组且长度为2，则直接拆成两个进行下一步递归
            if len(target_string) == 2:
                get_next_n_from_disc(target_string[0], disc_data, num, result_path, index_list, treat_type=treat_type)
                get_next_n_from_disc(target_string[1], disc_data, num, result_path, index_list, treat_type=treat_type)
            # 如果长度大于2，则拆成出第一个和其他进行下一步递归
            else:
                get_next_n_from_disc(target_string[0], disc_data, num, result_path, index_list, treat_type=treat_type)
                get_next_n_from_disc(target_string[1:], disc_data, num, result_path, index_list, treat_type=treat_type)
...
def basic_cut_data(data, cut_num=0, cut_rate=None, bins=None, remove_num=-1, cut_type='qcut'):
    """
...
def basic_Min_Max(value, min_value, max_value):
    if value < min_value:
        return 0
...
def get_fitter(data, start_rate=0.025, end_rate=0.975):
    """
    ...
    :return:
    """
    # 通过fitter对象，进行六种分布函数的拟合，拟合时长上限为100s
    fitter = Fitter(data, distributions=['gamma', 'rayleigh', 'norm', 'erlang', 'logistic', 'dgamma',
                                         'genlogistic', 'loglaplace', 'burr12', 'johnsonsu', 'dweibull'], timeout=100)
    # 进行拟合训练
    fitter.fit()
    # 得到拟合度最好的函数的参数
    key_parameter = fitter.get_best()
    # 得到该函数的名称
    key_f = list(key_parameter)[0]
    # 得到该函数的参数
    parameter_list = list(key_parameter[key_f].values())
    # 进行函数名称的判断，并进行相应函数的95%置信度下的临界值
    start_value = 0
    end_value = 0
    x_value = 0
    y_value = 0
    if key_f == 'gamma':
        start_value = gamma.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = gamma.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = gamma.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = gamma.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [gamma.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]
    elif key_f == 'rayleigh':
        start_value = rayleigh.ppf(start_rate, parameter_list[0], parameter_list[1])
        end_value = rayleigh.ppf(end_rate, parameter_list[0], parameter_list[1])

        start = rayleigh.ppf(0.001, parameter_list[0], parameter_list[1])
        end = rayleigh.ppf(0.999, parameter_list[0], parameter_list[1])
        x_value = np.linspace(start, end, 200)
        y_value = [rayleigh.pdf(value, parameter_list[0], parameter_list[1]) for value in x_value]
    elif key_f == 'norm':
        start_value = norm.ppf(start_rate, parameter_list[0], parameter_list[1])
        end_value = norm.ppf(end_rate, parameter_list[0], parameter_list[1])

        start = norm.ppf(0.001, parameter_list[0], parameter_list[1])
        end = norm.ppf(0.999, parameter_list[0], parameter_list[1])
        x_value = np.linspace(start, end, 200)
        y_value = [norm.pdf(value, parameter_list[0], parameter_list[1]) for value in x_value]
    elif key_f == 'erlang':
        start_value = erlang.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = erlang.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = erlang.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = erlang.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [erlang.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]
    elif key_f == 'logistic':
        start_value = logistic.ppf(start_rate, parameter_list[0], parameter_list[1])
        end_value = logistic.ppf(end_rate, parameter_list[0], parameter_list[1])

        start = logistic.ppf(0.001, parameter_list[0], parameter_list[1])
        end = logistic.ppf(0.999, parameter_list[0], parameter_list[1])
        x_value = np.linspace(start, end, 200)
        y_value = [logistic.pdf(value, parameter_list[0], parameter_list[1]) for value in x_value]
    elif key_f == 'dgamma':
        start_value = dgamma.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = dgamma.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = dgamma.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = dgamma.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [dgamma.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]

    elif key_f == 'genlogistic':
        start_value = genlogistic.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = genlogistic.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = genlogistic.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = genlogistic.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [genlogistic.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]

    elif key_f == 'loglaplace':
        start_value = loglaplace.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = loglaplace.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = loglaplace.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = loglaplace.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [loglaplace.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]

    elif key_f == 'burr12':
        start_value = burr12.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])
        end_value = burr12.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])

        start = burr12.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])
        end = burr12.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])
        x_value = np.linspace(start, end, 200)
        y_value = [burr12.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3]) for
                   value in x_value]

    elif key_f == 'johnsonsu':
        start_value = johnsonsu.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2],
                                    parameter_list[3])
        end_value = johnsonsu.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])

        start = johnsonsu.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])
        end = johnsonsu.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3])
        x_value = np.linspace(start, end, 200)
        y_value = [johnsonsu.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2], parameter_list[3]) for
                   value in x_value]

    elif key_f == 'dweibull':
        start_value = dweibull.ppf(start_rate, parameter_list[0], parameter_list[1], parameter_list[2])
        end_value = dweibull.ppf(end_rate, parameter_list[0], parameter_list[1], parameter_list[2])

        start = dweibull.ppf(0.001, parameter_list[0], parameter_list[1], parameter_list[2])
        end = dweibull.ppf(0.999, parameter_list[0], parameter_list[1], parameter_list[2])
        x_value = np.linspace(start, end, 200)
        y_value = [dweibull.pdf(value, parameter_list[0], parameter_list[1], parameter_list[2]) for value in x_value]

    return start_value, end_value, x_value, y_value
        ...
        with open(path) as file:
            for i, row in enumerate(file):
                ...
                if i == 0:  # 如果是第一行，获取到对比列的索引
                    col_index = get_index_of_list(row, [feature])
                else:
                    if row[col_index[0]] == '':
                        continue
                    if if_max:
                        if max_value < float(row[col_index[0]]):
                            max_value = float(row[col_index[0]])
                        elif min_value > float(row[col_index[0]]):
                            min_value = float(row[col_index[0]])
                    feature_data.append(float(row[col_index[0]]))

    if if_max:
        return feature_data, max_value, min_value
    else:
        return feature_data
...
'''
    ...
    修改时间：2022/6/30, change the start_station and end_station from first tolerate to station id
'''
            ...
            try:
                ...
                for value in standard:
                    if value == origin_list[0] or value[:-2] == origin_list[0][:-2]:
                        ...
                        now_value = value
                        break
                    elif {value[:-2]}.intersection(set([charge[:-2] for charge in origin_list_charge])):
                        standard_list.append(value)
                        if value[-2:] == '10':
    ...
    for path in paths:
        ...
        with open(path) as file:
            for i, row in enumerate(file):
                ...
                row[-1] = row[-1][:-1]
                if i == 0:  # 如果是第一行，获取到对比列的索引
                    col_index = get_index_of_list(row, col_name)
                else:
...
def compute_dict_by_group(data, index, treat_type, split_sign, filter_index=None, filter_value=None):
    """
    进行字典类型数据的内部汇总计算
    ...
    for key in data.keys():
        ...
        if treat_type == 'count':
            ...
            if type(data[key]) == list:
                try:
                    result_dict[new_key] = list(np.array(result_dict[new_key]) + np.array(data[key]))
                except:
                    result_dict[new_key] = result_dict[new_key]
            else:
                try:
        ...
        elif treat_type == 'list':
            try:
                result_dict[new_key].append(data[key])
            except:
                result_dict[new_key] = [data[key]]
...
'''
    创建时间：2023/2/17
    完成时间：2023/2/17
    功能：进行字典key中各特征相互占比的分析
    关键字：
    修改时间：
'''


def get_rate_of_dict_key(input_dict, rate_feature, target_feature):
    """
    进行字典key中各特征相互占比的分析
    :param input_dict: 输入字典
    :param rate_feature: 计算占比的特征
    :param target_feature: 底基特征
    :return:
    """
    # 计算底基特征的总量
    target_total_data = compute_dict_by_group(input_dict, target_feature, 'sum', '_')
    # 计算占比特征的总量
    target_feature.extend(rate_feature)
    rate_total_data = compute_dict_by_group(input_dict, target_feature, 'sum', '_')
    # 进行占比计算
    for key in rate_total_data.keys():

        rate_total_data[key] = [rate_total_data[key], rate_total_data[key] / target_total_data[
            key.rsplit('_', len(rate_feature))[0]]]

    return rate_total_data
...
def compute_distance_of_two_list(treat_type, list1, list2, parameter_list):
    """
    两列数据相似度计算，应用基于形状的计算：Hausdorff(豪斯多夫距离)、Fréchet(弗雷歇距离)；基于分段的计算：OWD(单向距离)、
    LIP(多线位置距离)；基于点的计算：CS(向量空间余弦相似度)、LCSS(最长公共子序列)、DTW(动态时间归整算法)、ED(欧几里得距离)
    :param dict parameter_list: 计算所需的参数集合
    ...
    if treat_type == 'Hausdorff':
        dtw.dtw(list1, list2, parameter_list['dist'], parameter_list['warp'], parameter_list['w'], parameter_list['s'])

    # DTW(动态时间归整算法)
    elif treat_type == 'DTW':
        # 计算得到两条数据的相似度值
        dict_value, cost, acc, path = dtw.dtw(list1, list2, parameter_list['dist'], parameter_list['warp'],
                                              parameter_list['w'], parameter_list['s'])
...
def get_feature_random(features, random_rate=None, random_type='', feature_type='str'):
    """
    ...
    if feature_type == 'str':
        # 将概率集的值转换成从0到1的累加值
        random_rate_new = [sum(random_rate[:(i + 1)]) if i != (len(random_rate) - 1) else sum(random_rate[:]) for i in
                           range(len(random_rate))]
        # 获取0到1之间的随机数
...
'''
    创建时间：2022/11/29
    完成时间：2022/11/29
    功能：判断输入的第二个元素是否包含第一个元素
    关键词：包含，判断
    修改时间：
'''


def charge_inputs_contain(input_value1, input_value2):
    """
    判断输入的第二个元素是否包含第一个元素
    :param input_value1: 输入参数1
    :param input_value2: 输入参数2
    :return:
    """
    if input_value1 in input_value2:
        return True
    else:
        return False
...
def charge_inputs_equal(input_value1, input_value2):
    """
    两个元素是否相等的判断
    :param input_value1: 输入参数1
    :param input_value2: 输入参数2
...
'''
    创建时间：2022/11/29
    完成时间：2022/11/29
    功能：判断输入的第一个是否大于第二个元素
    关键词：大于，判断
    修改时间：
'''


def charge_inputs_bigger(input_value1, input_value2):
    """
    判断输入的第一个是否大于第二个元素
    :param input_value1: 输入参数1
    :param input_value2: 输入参数2
...
'''
    创建时间：2022/11/29
    完成时间：2022/11/29
    功能：判断输入的第一个是否大于等于第二个元素
    关键词：大于，判断
    修改时间：
'''


def charge_inputs_bigger_and_equal(input_value1, input_value2):
    """
    判断输入的第一个是否大于等于第二个元素
    :param input_value1: 输入参数1
    :param input_value2: 输入参数2
...
'''
    创建时间：2023/2/16
    完成时间：2023/2/16
    功能：读取文件内容，并返回list类型数据
    关键词：读取，list
    修改时间：
'''


def get_list_from_document(read_path):
    """
    读取文件内容，并返回list类型数据
    :param str read_path: 文件读取地址
    :return:
    """
    result_data = []  # 用于保存读取后数据
...
'''
    创建时间：2023/2/16
    完成时间：2023/2/16
    功能：读取项目配置文件，并返回dict类型数据
    关键词：配置文件，dict
    修改时间：
'''


def get_dict_from_config(read_path, sign=':'):
    """
    读取文件内容，并返回list类型数据
    :param sign: 拆分字符
    :param str read_path: 文件读取地址
...
def concat_dict(input_dicts, concat_type):
    """
    进行两个字典的匹配合并
    :param list input_dicts: 输入的字典集，可以为多个
    :param str concat_type: 合并的方式，如相加、相减、拼接等
    :return:
    """
    concat_data = {}  # 用于合并所有字典
    # 遍历所有的字典，进行合并
    for i in range(len(input_dicts)):
        # 判断当前是否是最后一个，如果是直接跳出循环
        if i == (len(input_dicts) - 1):
            # 获取两个字典的最小公key集
            first_dict_keys = set(input_dicts[i].keys())
            second_dict_keys = set(input_dicts[i + 1].keys())
            min_keys_list = list(first_dict_keys.intersection(second_dict_keys))

            for key in min_keys_list:
                # 元素相加
                if concat_type == 'add':
                    concat_data[key] = input_dicts[i][key] + input_dicts[i + 1][key]
                # 数组合并
                elif concat_type == 'append':
                    concat_data[key] = input_dicts[i][key] + input_dicts[i + 1][key]
                # 数组合并
                elif concat_type == 'list':
                    concat_data[key] = [input_dicts[i][key], input_dicts[i + 1][key]]
                # 数组元素相加
                elif concat_type == 'element_add':
                    for j in range(len(input_dicts[i][key])):
                        try:
                            concat_data[key][j] += input_dicts[i + 1][key][j]
                        except:
                            if j == 0:
                                concat_data[key] = []
                            concat_data[key].append(input_dicts[i][key][j] + input_dicts[i + 1][key][j])
        else:
            break
    return concat_data
...
def sort_dict(input_dict, target, reverse=True):
    """
    进行两个字典的匹配合并
    :param target: 进行排序的对象，key为根据key值排序，value为根据value值排序
    :param dict input_dict: 输入的字典
    ...
    if target == 'key':
        ...
        index_value = 1
    # 对字典进行排序，输出元组数组
    sorted_list = sorted(input_dict.items(), key=lambda x: x[index_value], reverse=reverse)
    # 根据排序好的元组进行字典重组
    new_dict = {}
    for i in range(len(sorted_list)):
        new_dict[sorted_list[i][0]] = sorted_list[i][1]
    return new_dict
...
'''
    创建时间：2023/2/18
    完成时间：2023/2/18
    功能：从字典中筛选数据，key包含target_value的进行返回
    关键词：字典，筛选
    修改时间：
'''


def get_part_dict(input_dict, target_value):
    """
    进行两个字典的匹配合并
    :param dict input_dict: 输入的字典
    :param bool target_value: 过滤的内容
    :return:
    """
    # 遍历字典key，进行判断
    filter_dict = {}
    for key in input_dict.keys():
        if target_value in key:
            filter_dict[key] = input_dict[key]
    return filter_dict
